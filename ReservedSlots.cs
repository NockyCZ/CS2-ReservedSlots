...
public class ReservedSlotsConfig : BasePluginConfig
{
    ...
    [JsonPropertyName("Use Discord Utilities")] public bool useDiscordUtilities { get; set; } = false;
...
public class ReservedSlots : BasePlugin, IPluginConfig<ReservedSlotsConfig>
{
    ...
    public override void OnAllPluginsLoaded(bool hotReload)
    {
        if (Config.useDiscordUtilities)
        {
            try
            {
                ...
                {
                    Config.useDiscordUtilities = false;
                    Console.WriteLine($"[Reserved Slots] Discord Utilities plugin was not found! (https://github.com/NockyCZ/CS2-Discord-Utilities)");
                }
            }
            catch (Exception ex)
            {
                Config.useDiscordUtilities = false;
                Console.WriteLine($"[Reserved Slots] An error occurred while loading Discord Utilities: {ex.Message}");
            }
        }
    }
    ...
    {
        if (Config.useDiscordUtilities && DiscordUtilities != null)
        {
